This folder contains two actual programs: Raycaster.py and Raycasting_Engine.py. Both
...
PC at the time, so I wanted to work with Python. I essentially translated the code from 
C to Python. When the program is run, a Raycasted view is shown in the right half of the
...
to, so that when one maze is solved, another one loads instantly.

These two programs, though rudimentary, were the first proper game engines I've ever worked on,
and remain some of my favorite projects that I've ever worked on.